...
public class AnyframeMavenLaunchDelegate extends JavaLaunchDelegate {
    ...
    public void launch(ILaunchConfiguration configuration, String mode,
        ...
        File state = AnyframeIDEPlugin.getDefault().getStateLocation().toFile();
        try {
            File dir = new File(state, "launches");
            dir.mkdirs();
            confFile = File.createTempFile("m2conf", ".tmp", dir);
            launch
                .setAttribute(LAUNCH_M2CONF_FILE, confFile.getCanonicalPath());
            ...
            throw new CoreException(new Status(IStatus.ERROR,
                AnyframeIDEPlugin.ID, -1, "Can't create m2.conf ", e));
        }
    ...
    public String[] getClasspath(ILaunchConfiguration configuration)
        ...
        List<String> cp =
            m2conf.getRealmEntries(IMavenLauncherConfiguration.LAUNCHER_REALM);
        ...
        return new IVMRunner() {
            public void run(VMRunnerConfiguration runnerConfiguration,
                ...
                if (processes != null && processes.length > 0) {
                    AnyframeBackgroundResourceRefresher refresher =
                        new AnyframeBackgroundResourceRefresher(configuration,
                            launch);
                    refresher.init();
                } else {
