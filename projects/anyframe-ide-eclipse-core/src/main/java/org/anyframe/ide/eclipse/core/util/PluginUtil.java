...
public class PluginUtil {
    ...
    private static org.anyframe.ide.command.ant.task.collector.PluginCollector antCollector =
        null;
    private static org.anyframe.ide.command.maven.mojo.collector.PluginCollector mavenCollector =
        null;
    ...
    public static Map<String, PluginInfo> getPluginList(String pjtBuild,
        ...
        if (pjtBuild.equals(CommonConstants.PROJECT_BUILD_TYPE_ANT)) {
            ...
            antCollector =
                new org.anyframe.ide.command.ant.task.collector.PluginCollector(
                    anyframeHome, baseDir, isOffline);

            PluginContainer container = antCollector.getContainer();
            WagonManager wagonManager =
                (WagonManager) container.lookup(WagonManager.class.getName());

            ExceptionUtil.showException(
                "Anyframe IDE Editor PluginUtil (getPluginList) offline mode : "
                    + isOffline
                    + ", wagonManager isOnline : "
                    + wagonManager.isOnline()
                    + ", remote repositories size : "
                    + container.getRequest().getRemoteArtifactRepositories()
                        .size(), IStatus.INFO, null);

            return antCollector.getPlugins(baseDir);
        } else {
            mavenCollector =
                new org.anyframe.ide.command.maven.mojo.collector.PluginCollector(
                    baseDir);
    ...
    public static String getLatestArchetypeVersion(String archetypeArtifactId,
        ...
        if (pjtBuild.equals(CommonConstants.PROJECT_BUILD_TYPE_ANT)) {
            ...
            ExceptionUtil.showException(
                "Anyframe IDE Editor PluginUtil (getLatestArchetypeVersion) offline mode : "
                    + isOffline
                    + ", wagonManager isOnline : "
                    + wagonManager.isOnline()
                    + ", remote repositories size : "
    ...
    public static List<String> getArchetypeVersions(String groupId,
        ...
        if (pjtBuild.equals(CommonConstants.PROJECT_BUILD_TYPE_ANT)) {
            ...
            ExceptionUtil.showException(
                "Anyframe IDE Editor PluginUtil (getArchetypeVersions) offline mode : "
                    + isOffline
                    + ", wagonManager isOnline : "
                    + wagonManager.isOnline()
                    + ", remote repositories size : "
    ...
    public static String checkInstall(String pjtBuild, String anyframeHome,
        ...
        if (pjtBuild.equals(CommonConstants.PROJECT_BUILD_TYPE_ANT)) {
            ...
            PluginContainer container = antCollector.getContainer();
            installer =
                (DefaultPluginInstaller) container
            ...
            ExceptionUtil.showException(
                "Anyframe IDE Editor PluginUtil (checkInstall) offline mode : "
                    + isOffline
                    + ", wagonManager isOnline : "
                    + wagonManager.isOnline()
                    + ", remote repositories size : "
            ...
            org.anyframe.ide.command.maven.mojo.container.PluginContainer container =
                mavenCollector.getContainer();
            installer =
                (DefaultPluginInstaller) container
                    .lookup(DefaultPluginInstaller.class.getName());
            request = container.getRequest();
        }
        Map<String, TargetPluginInfo> visitedPlugins =
            new HashMap<String, TargetPluginInfo>();
    ...
    public static Map<String, PluginInfo> getInstalledPluginList(
        ...
        if (pjtBuild.equals(CommonConstants.PROJECT_BUILD_TYPE_ANT)) {
            ...
            ExceptionUtil.showException(
                "Anyframe IDE Editor PluginUtil (getInstalledPluginList) offline mode : "
                    + isOffline
                    + ", wagonManager isOnline : "
                    + wagonManager.isOnline()
                    + ", remote repositories size : "
    ...
    public static Set<String> getPluginNameSet(String pjtBuild,
        ...
        if (pjtBuild.equals(CommonConstants.PROJECT_BUILD_TYPE_ANT)) {
            org.anyframe.ide.command.ant.task.collector.PluginCollector collector =
                new org.anyframe.ide.command.ant.task.collector.PluginCollector(
                    anyframeHome, null, isOffline);
            return collector.getPluginNames();
        } else {
            org.anyframe.ide.command.maven.mojo.collector.PluginCollector collector =
                new org.anyframe.ide.command.maven.mojo.collector.PluginCollector(
                    null);
            return collector.getPluginNames();
        }
    ...
    private static boolean isOffline(String baseDir) throws Exception {
        // get offline option
        String properties =
            baseDir + CommonConstants.METAINF + CommonConstants.METADATA_FILE;
        PropertiesIO appProps = new PropertiesIO(properties);
        boolean offlineValue =
            new Boolean(appProps.readValue(CommonConstants.OFFLINE))
                .booleanValue();
