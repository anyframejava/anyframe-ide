...
/**
 * This is a PluginUninstallPostProcess class.
 * @author Changje Kim
 * @author Sooyeon Park
 */
public class PluginUninstallPostProcess implements PostProcess {

    private CommandVO vo = null;

    public PluginUninstallPostProcess(CommandVO vo) {
        this.vo = vo;
    }
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                Display.getDefault().asyncExec(new Runnable() {
                    public void run() {
                        UninstallPluginVO pluginVO = (UninstallPluginVO)vo;
                        CodeGenEditor ideEditor = pluginVO.getIdeEditor();
                        
                        ((ConfigPage)ideEditor.getMapPages().get(2)).refreshAction();

                        InstallationPage installPage = (InstallationPage)ideEditor.getMapPages().get(4);
