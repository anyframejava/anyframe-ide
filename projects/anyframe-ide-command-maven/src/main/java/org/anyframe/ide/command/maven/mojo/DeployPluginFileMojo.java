...
public class DeployPluginFileMojo extends AbstractPluginMojo {
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		try {
			...
			if (server == null) {
				// server is null - authentication is not required
				// throw new CommandException(
			...
			Proxy proxy = super.settings.getActiveProxy();
			...
			pluginCatalogManager.updatePluginCatalog(request, baseDir, url,
					userName, password, proxyHost,
					proxyPort, pluginInfo.isEssential(), isLatest, pluginInfo);
		} catch (Exception ex) {
			getLog().error(
