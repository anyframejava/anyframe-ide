...
<#if field.equals(pojo.identifierProperty)>
	<#if !c2j.isComponent(field) >
    	<#lt/>			<td class="underline">
				<a class="linkClass" href="${r"${ctx}"}/${pojoNameLower}.do?method=get&${field.name}=${r"$"}{${pojoNameLower}.${field.name}}">${r"$"}{${pojoNameLower}.${field.name}}</a>
			</td>
	<#else>		        	    
		<#assign pojoIdentifier = pojo.identifierProperty.getValue() >
		<#assign keyList = dbdata.getKeyList(pojo)/>
		<#foreach idfield in keyList.keyList().iterator()>          		 	
			...
			<#if keyList.get(idfield) != "FK" >
    	<#lt/>			<td class="underline">
				<a class="linkClass" href="${r"${ctx}"}/${pojoNameLower}.do?method=get<#foreach idfield in pojoIdentifier.getPropertyIterator()>&${field.name}.${idfield.name}=${'$'}{${pojoNameLower}.${field.name}.${idfield.name}}</#foreach>">${r"$"}{${pojoNameLower}.${field.name}.${idfield.name}}</a>
			</td>
  			</#if>
  		</#foreach>     			
	</#if>
<#elseif !c2h.isCollection(field) && !c2h.isManyToOne(field) && !c2j.isComponent(field)>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top: 10px;">
		<tr>
			<td align="right"><a href='<spring:url value="javascript:fncCreate${pojo.shortName}View();" htmlEscape="true"/>'><img
				src="<spring:url value='/sample/images/btn_add.png'/>" width="64" height="18" border="0" /></a></td>
		</tr>
	</table>
</form:form>