...
<#if field.equals(pojo.identifierProperty)>
	<#if !c2j.isComponent(field) >
            <td><a href="${r"${ctx}"}/${pojoNameLower}.do?method=get&amp;${field.name}=${r"$"}{${pojoNameLower}.${field.name}}">${r"$"}{${pojoNameLower}.${field.name}}</a></td>
	<#else>		        	    
		<#assign pojoIdentifier = pojo.identifierProperty.getValue() >
		<#assign keyList = dbdata.getKeyList(pojo)/>
		<#foreach idfield in keyList.keyList().iterator()>          		 	
			<#assign columnName = dbdata.getColumnName(idfield)>
			<#if keyList.get(idfield) != "FK" >
            <td><a href="${r"${ctx}"}/${pojoNameLower}.do?method=get<#foreach idfield in pojoIdentifier.getPropertyIterator()>&amp;${field.name}.${idfield.name}=${'$'}{${pojoNameLower}.${field.name}.${idfield.name}}</#foreach>">${r"$"}{${pojoNameLower}.${field.name}.${idfield.name}}</a></td>
  			</#if>
  		</#foreach>     			
	</#if>
<#elseif !c2h.isCollection(field) && !c2h.isManyToOne(field) && !c2j.isComponent(field)>
