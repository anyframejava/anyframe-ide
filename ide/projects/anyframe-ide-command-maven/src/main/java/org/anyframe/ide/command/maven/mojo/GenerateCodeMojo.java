...
public class GenerateCodeMojo extends AbstractPluginMojo {
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		try {
			setConfiguration();
			// modify by junghwan.hong 
			// 1. template type check
			if("".equals(this.templateType)) {
				throw new CommandException("Do not insert template type information. Please insert template type.");
			}
			// 2. core installed check
			checkInstalledPlugins(new String[] { CommonConstants.CORE_PLUGIN });
			// 3. template type installed check
			if(! "springjdbc".equals(this.templateType)) {
				checkInstalledPlugins(new String[] { this.templateType });
			}
			...
			getLog().error("Fail to execute GenerateCodeMojo. The reason is '" + ex.getMessage() + "'.", ex);
