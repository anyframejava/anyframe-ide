...
public class GenerateModelMojo extends AbstractPluginMojo {
	...
	private static String DECIMAL_DIGITS = "DECIMAL_DIGITS";
	private static String NULLABLE = "IS_NULLABLE";
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		try {
			...
			getLog().error("Fail to execute GenerateModelMojo. The reason is '" + ex.getMessage() + "'.", ex);
	...
	public Domain generateModel(String strTable) throws Exception {
		...
		while (columns.next()) {
			...
			column.setLength(String.valueOf(columns.getInt(COLUMN_SIZE) - columns.getInt(DECIMAL_DIGITS)));
