...
public class HudsonRemoteAPI {
	...
	public void saveHudsonConfig(String antHome, String mavenHome,
		...
		PostMethod method = new PostMethod(hudsonURL
	...
	public void createJob(String jobName, String type, Context context)
		...
		PostMethod method = new PostMethod(hudsonURL + "createItem?name="
				+ jobName);
	...
	private void changeXmlElementWithUserInput(Context context) {
		...
		changeScmElement((String) context.get("scmType"),
				(String) context.get("scmUrl"));
	...
	private void changeChlidProjectElement(String childProjectText) {
		Element childProject = jobConfigElement.getChild("publishers")
				.getChild("hudson.tasks.BuildTrigger");
		if (childProject == null) {
			if (!"".equals(childProjectText)) {
				Element child = createXmlElement(
	...
	private void changeTriggerElement(String scheduleText) {
		Element trigger = jobConfigElement.getChild("triggers").getChild(
				"hudson.triggers.SCMTrigger");
		if (trigger == null) {
			if (!"".equals(scheduleText)) {
				Element child = createXmlElement(
						"hudson.triggers.SCMTrigger/spec", scheduleText);
	...
	public void executeBuild(String jobName) throws Exception {
		...
		GetMethod method = new GetMethod(hudsonURL + "job/" + jobName
				+ "/build");
		try {
			HttpClient httpClient = new HttpClient();
			int statusCode = httpClient.executeMethod(method);
	...
	public void deleteJob(String jobName) throws Exception {
		...
		PostMethod method = new PostMethod(hudsonURL + "job/" + jobName
				+ "/doDelete");
		try {
			HttpClient httpClient = new HttpClient();
			int statusCode = httpClient.executeMethod(method);
