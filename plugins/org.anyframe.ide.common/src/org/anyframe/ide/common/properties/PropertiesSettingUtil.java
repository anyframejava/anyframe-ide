...
public class PropertiesSettingUtil {
	...
	public static void parsing(String pjtLoc) {
		...
		try {
			if (file.exists()) {
				...
				NodeList template = root
						.getElementsByTagName(Constants.XML_TAG_TEMPLATE);
				NodeList jdbcdrivers = root
						.getElementsByTagName(Constants.XML_TAG_JDBCDRIVERS);
				NodeList databases = root
						.getElementsByTagName(Constants.XML_TAG_DATABASES);
			...
			PluginLoggerUtil.error(CommonActivator.PLUGIN_ID,
					Message.exception_load_config_xml, e);
		}
	}

	// save
	public static boolean save(String pjtLoc, CommonPropertiesVO vo) {
		boolean result = true;
		...
		try {
			...
			Element rootElement = doc.createElement(Constants.XML_COMMON_ROOT);
			doc.appendChild(rootElement);

			Node template = doc.createElement(Constants.XML_TAG_TEMPLATE);
			rootElement.appendChild(template);
			template.appendChild(doc.createTextNode(vo.getTemplateLoc()));

			Node jdbcdrivers = doc.createElement(Constants.XML_TAG_JDBCDRIVERS);
			rootElement.appendChild(jdbcdrivers);
			jdbcdrivers.appendChild(doc.createTextNode(vo.getJdbcdriversLoc()));

			Node databases = doc.createElement(Constants.XML_TAG_DATABASES);
			rootElement.appendChild(databases);
			databases.appendChild(doc.createTextNode(vo.getDatabasesLoc()));

			File file = new File(commonXml);
			if(!file.exists()){
				file.getParentFile().mkdirs();
			}
			XMLUtil.save(doc, file);
			
		} catch (Exception e) {
			result = false;
			PluginLoggerUtil.error(CommonActivator.PLUGIN_ID,
					Message.exception_save_config_xml, e);
		}
	...
	public static String getJdbcdriversFile(String pjtLoc) {
		...
		return JDBCDRIVERS_LOC + Constants.FILE_SEPERATOR
	...
	public static String getDatabasesFile(String pjtLoc) {
		...
		return DATABASES_LOC + Constants.FILE_SEPERATOR
				+ Constants.DB_SETTINGS_XML_FILE;
	}

	// check and setting default
	public static String getPrefs(String pjtLoc) {

		String propertyFile = pjtLoc + Constants.FILE_SEPERATOR
				+ Constants.COMMON_CONFIG_PREFS_FILE;
		...
		if (!file.exists()) {
			...
			try {
				...
				PluginLoggerUtil.info(CommonActivator.PLUGIN_ID,
						Message.exception_save_properties + e.getMessage()
								+ ".");
			} finally {
				try {
					if (out != null){
	...
	private static boolean create(String pjtLoc) {
		...
		vo.setTemplateLoc(pjtLoc + Constants.FILE_SEPERATOR
				+ Constants.DFAULT_TEMPLATE_HOME);
		vo.setJdbcdriversLoc(pjtLoc + Constants.FILE_SEPERATOR
				+ Constants.SETTING_HOME);
		vo.setDatabasesLoc(pjtLoc + Constants.FILE_SEPERATOR
				+ Constants.SETTING_HOME);
	...
	private static String getDefaultCommonXml(String pjtLoc) {
		return pjtLoc + Constants.FILE_SEPERATOR + Constants.SETTING_HOME
				+ Constants.FILE_SEPERATOR + Constants.COMMON_SETTINGS_XML_FILE;
	}
